...
def generate_diplomas():
    ...
    try:
        ...
        data = list(reader)
        # Создаем переменную для отслеживания ошибок она равна 2, чтобы пользователю было легче искать в файле с данными
        index_row = 2
        ...
        for row in data:
            ...
            # Выделяем фамилию, имя с отчеством
            fio = row['ФИОСлушателя'].split()
            lastname = ''
            firstname_patr = ''
            if fio == 3:
                lastname = fio[0]
                firstname_patr = ' '.join([fio[1], fio[2]])
            elif fio == 2:
                lastname = fio[0]
                firstname_patr = fio[1]
            else:
                lastname = fio[0]
            context = {'ФИОСлушателя': row['ФИОСлушателя'], 'Фамилия': lastname, 'НомерДок': number,
                       'ИмяОтчество': firstname_patr, 'ФИОСлушателяРодПадеж': row['ФИОСлушателяРодПадеж'],
        ...
        messagebox.showinfo('Dodger', f'Произошла ошибка при обработке строки {index_row} ')
    ...
    try:
        ...
        for row in data:
            ...
            if len(fio) == 3:
                ...
                lastname = fio[0]
                firstname_patr = ''
            context = {'ФИОСлушателя': row['ФИОСлушателя'], 'Фамилия': lastname, 'НомерДок': row['НомерДок'],
                       'ИмяОтчество': firstname_patr, 'ФИОСлушателяРодПадеж': row['ФИОСлушателяРодПадеж'],
                       'Оператор': row['Оператор'], 'РегионРодПадеж': row['РегионРодПадеж'],
                       'ВидПрограммы': row['ВидПрограммы'], 'ПодвидПрограммы': row['ПодвидПрограммы'],
                       'ФорматПрограммы': row['ФорматПрограммы'], 'НазваниеПрограммы': row['НазваниеПрограммы'],
